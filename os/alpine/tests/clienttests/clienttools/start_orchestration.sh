...
curl -v -s --insecure --cert-type P12 --cert "${systemName}.p12":123456 -X POST -H "Content-Type: application/json" -d @orchestration.request https://${orchestrator_address}:${orchestrator_port}${orchestrator_uri} > $out_file
